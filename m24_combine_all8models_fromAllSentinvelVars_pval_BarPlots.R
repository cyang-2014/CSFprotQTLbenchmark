...
### make sure the model-order to match with the color-code
long.modelAll8.dt <- data.table::melt(modelAll8.dt[, list(protVarBlockID, LOG10_P.T01, LOG10_P.T02,
                                                          LOG10_P.T03, LOG10_P.T04,
                                                          LOG10_P.T05, LOG10_P.T06,
                                                          LOG10_P.T07, LOG10_P.T08)], id.vars = 1)
...
long.modelAll8.dt %>%
  ...
  summarise(count_pqtl = n()) %>%
  arrange(-count_pqtl) %>%
  ggplot(aes(x = fct_rev(fct_reorder(variable, count_pqtl)), y = count_pqtl, fill=variable)) +
  geom_col() +
...
ggplot(trans.gw.compare, aes(x = fct_rev(fct_reorder(variable, ratio.Odds)), y = ratio.Odds, fill=variable)) +
  geom_col() +
  scale_fill_manual(values=c("darkred", "red", "orange", "yellow",
                             "lightgreen", "green", "lightblue", "blue"))+
  theme_cowplot(font_size = 8) +
